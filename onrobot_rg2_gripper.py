...
class OnRobotGripperRG2(object):
    ...
    def close_gripper(self, target_width=-10, target_force=40, payload=0.5, set_payload=False, depth_compensation=False, slave=False, wait=0.5):
        while(self.gripper_position() > 0.5):
            urscript = OnRobotGripperRG2Script()
            urscript._rg2_command(target_width, target_force, payload, set_payload, depth_compensation, slave)
            self.robot.send_program(urscript())
            time.sleep(wait)
            if self.gripper_position() > 0.5:
                print('error closing gripper, retrying')
